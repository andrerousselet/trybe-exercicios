...
class FrameService implements IService<IFrame> {
  ...
  public async create(obj: unknown): Promise<IFrame> {
    ...
    if (!parsed.success) {
      throw parsed.error; // vamos falar sobre como tratar esse erro logo logo
    }
  ...
  public async read(): Promise<IFrame[]> {
    const frameList = await this._frame.read();
    if (!frameList) throw new Error(ErrorTypes.EntityNotFound);
    return frameList;
  }
}
