...
describe('Frame Controller', () => {
  ...
  describe('ReadOne Frame', () => {
    it('Success', async () => {
      // como fizemos o dublê da service, o valor do `req.params.id` não vai chegar na model, logo ele só precisa ser uma string e existir
  ...
  describe('Read Frames', () => {
    it('Success', async () => {
      ...
      expect((res.status as sinon.SinonStub).calledWith(200)).to.be.true;
      expect((res.json as sinon.SinonStub).calledWith(framesWithIdListMock)).to.be.true;
    });
  });
});